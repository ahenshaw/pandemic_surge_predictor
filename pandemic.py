...
from mysecrets import BEARER_TOKEN
...
def get_tweets_from_near_location(
    keywords,
    lat,
    lon,
    radius=25,
    start_time="2020-01-01T00:00:00Z",
    end_time="2022-03-30T00:00:00Z",
):
    params = {
    ...
    for page in pages:
        try:
            ...
            results.extend(
                [
                    (
                        datetime.strptime(x["start"], "%Y-%m-%dT%H:%M:%S.000Z"),
                        x["tweet_count"],
                    )
                    for x in data
                ]
...
if __name__ == "__main__":
    ...
    count_fetched = 1
    cursor.execute(
        "SELECT id, lat, lng, county_ascii, state_id from county where state_id=? order by state_id, county_ascii",
        (STATE,),
    )
    for county_id, lat, lon, county_name, state in cursor.fetchall():
        if county_id not in seen:
            ...
            cursor.executemany(
                "INSERT INTO tweet_count (county_id, tdate, count) VALUES (?, ?, ?)",
                to_insert,
            )
            db.commit()
            count_fetched += 1
